...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .brand-sub { font-size: .7rem; font-weight: 500; color: var(--gray-400); margin-top: 2px; }
        .topbar-center {
            display: flex; gap: 6px;
            background: var(--gray-100); padding: 4px; border-radius: 48px;
        }
        .topbar-center a {
            padding: 8px 20px; font-size: .85rem; font-weight: 600;
            color: var(--gray-600); text-decoration: none; border-radius: 40px; transition: all .2s;
        }
        ...
        .user-pill {
            ...
            background: var(--gray-100); padding: 5px 12px 5px 8px; border-radius: 48px;
        }
        .user-avatar {
        ...
        .user-name { font-weight: 600; font-size: .85rem; color: var(--gray-800); }
        .btn-logout {
            background: transparent; border: 1px solid var(--gray-300); border-radius: 32px;
            padding: 7px 18px; font-weight: 600; font-size: .75rem; color: var(--gray-600);
        ...
        /* ── BREADCRUMB ── */
        .breadcrumb-bar {
            padding: 14px 32px; font-size: .75rem; color: var(--gray-400);
            background: transparent; max-width: 1280px; margin: 0 auto; width: 100%;
        }
        .breadcrumb-bar a { color: var(--primary); text-decoration: none; font-weight: 500; }

        /* ── LAYOUT ── */
        .page {
            max-width: 1320px; margin: 24px auto 48px; padding: 0 28px;
        ...
        .icon-wrap {
            width: 44px; height: 44px; background: var(--primary-soft);
            border-radius: 20px; display: flex; align-items: center; justify-content: center;
            color: var(--primary);
        }
        .panel-head h2 { font-size: 1rem; font-weight: 700; color: var(--gray-800); }
        ...
        .form-label {
            ...
            color: var(--gray-500); letter-spacing: .03em; margin-bottom: 8px; display: block;
        }
        .req { color: #e5484d; }
        .form-control, select.form-control, textarea.form-control {
            ...
            font-family: 'Inter', sans-serif; font-size: .85rem; transition: all .2s; outline: none;
            color: var(--gray-800);
        }
        ...
        .suggest-title { font-size: .85rem; font-weight: 700; color: var(--gray-800); }
        .suggest-sub   { font-size: .72rem; color: var(--gray-500); margin-left: auto; }
        ...
        .sug-rank.r1 { background: #f59e0b; }
        .sug-rank.r2 { background: #9ca3af; }
        .sug-rank.r3 { background: #cd7c5b; }
        ...
        .sug-avatar {
            ...
            margin: 0 auto 10px; overflow: hidden; border: 2px solid var(--primary-soft);
        }
        .sug-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .sug-name  { font-size: .82rem; font-weight: 800; color: var(--gray-800); }
        .sug-stars { color: #f59e0b; font-size: .72rem; margin: 4px 0; }
        .sug-tags  { display: flex; justify-content: center; gap: 6px; flex-wrap: wrap; margin-top: 8px; }
        .sug-tag {
            font-size: .62rem; font-weight: 700; padding: 2px 8px; border-radius: 100px;
        }
        .sug-tag.slot  { background: #d1fae5; color: #065f46; }
        .sug-tag.exp   { background: #dbeafe; color: #1d4ed8; }
        .sug-tag.price { background: #ede9fe; color: #6d28d9; }
        .sug-cta { display: none; font-size: .68rem; font-weight: 700; color: var(--primary); margin-top: 8px; }
        .suggest-card.selected-sug .sug-cta { display: block; }

        .suggest-loading {
            grid-column: 1 / -1;
            display: flex; align-items: center; gap: 10px;
            font-size: .8rem; color: var(--gray-500); padding: 8px 0;
        }
        ...
        .slot-stats-bar {
            ...
            margin-bottom: 12px; font-size: .72rem; color: var(--gray-500);
        }
        .slot-stat { display: flex; align-items: center; gap: 6px; font-weight: 600; }
        .slot-stat-dot { width: 8px; height: 8px; border-radius: 3px; }

        .slot-wrap { display: flex; flex-wrap: wrap; gap: 10px; margin: 12px 0 16px; }
        ...
        .slot-session-label::after { content: ''; flex: 1; height: 1px; background: var(--gray-200); }
        ...
        .slot-btn {
            ...
            min-width: 84px; color: var(--gray-700); font-family: 'Inter', sans-serif;
        }
        .slot-end { font-size: .6rem; font-weight: 400; color: var(--gray-400); }
        .slot-btn:hover:not(:disabled):not(.slot-full) {
            background: var(--primary-soft); border-color: var(--primary);
            transform: translateY(-2px); box-shadow: 0 6px 14px rgba(15,82,186,.15);
            color: var(--primary);
        }
        .slot-btn:hover:not(:disabled):not(.slot-full) .slot-end { color: var(--primary); opacity: .7; }
        .slot-btn.selected {
        ...
        .slot-btn.slot-full {
            background: #fef2f2; border-color: #ffc9c9; color: #e5484d;
            cursor: not-allowed; opacity: .7;
        }
        .slot-btn.slot-full .slot-end { color: #fca5a5; }

        .slot-legend { display: flex; gap: 20px; margin: 4px 0; flex-wrap: wrap; }
        .legend-item { font-size: .7rem; color: var(--gray-500); display: flex; align-items: center; gap: 8px; }
        .legend-dot { width: 12px; height: 12px; border-radius: 20px; background: var(--gray-200); border: 1px solid var(--gray-300); }
        ...
        .doctor-card, .summary-card {
            ...
            box-shadow: var(--shadow-lg); border: 1px solid rgba(0,0,0,.03); overflow: hidden;
        }
        .card-head {
            padding: 16px 24px; background: var(--gray-50);
            font-weight: 700; font-size: .7rem; letter-spacing: .04em;
            text-transform: uppercase; color: var(--primary); border-bottom: 1px solid var(--gray-200);
        }
        .doctor-body { padding: 24px; text-align: center; }
        .doc-avatar {
            width: 80px; height: 80px;
            background: linear-gradient(145deg, var(--primary), #448af2);
            border-radius: 50%; margin: 0 auto 14px;
            display: flex; align-items: center; justify-content: center;
            font-weight: 800; font-size: 1.6rem; color: white; overflow: hidden;
            box-shadow: 0 12px 18px -8px rgba(15,82,186,.3);
        }
        .doc-avatar img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
        .doc-name { font-size: 1.05rem; font-weight: 800; color: var(--gray-800); }
        .doc-bio  { font-size: .74rem; color: var(--gray-500); margin-top: 6px; line-height: 1.5; }
        .doc-stars { color: #f5b042; margin: 8px 0; font-size: .8rem; }
        .doc-meta-row {
            display: flex; justify-content: space-between; padding: 8px 0;
            font-size: .8rem; border-bottom: 1px dashed var(--gray-100);
        }

        .sum-body { padding: 18px 22px; }
        .sum-row {
            display: flex; justify-content: space-between; align-items: center;
            padding: 11px 0; border-bottom: 1px solid var(--gray-100); font-size: .82rem;
        }
        .sum-row:last-child { border-bottom: none; }
        .sum-row .k { color: var(--gray-500); }
        .sum-row .v { font-weight: 600; color: var(--gray-800); }
        .sum-row .v.empty { color: var(--gray-400); font-weight: 400; }
        .status-pill {
            background: #fff6e0; border-radius: 100px; padding: 4px 12px;
            font-size: .7rem; font-weight: 700; color: #c47b2e;
        }

        /* ── BUTTONS ── */
        .btn-row { display: flex; justify-content: flex-end; gap: 16px; margin-top: 32px; }
        .btn {
            padding: 12px 28px; border-radius: 48px;
            font-weight: 700; font-size: .85rem; transition: all .2s; cursor: pointer;
        ...
        .btn-primary {
            ...
            color: white; box-shadow: 0 4px 12px rgba(15,82,186,.3);
        }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 22px -6px rgba(15,82,186,.4); }
        ...
        .footer {
            ...
            text-align: center; padding: 28px; font-size: .75rem; color: var(--gray-500);
        }
...
<div style="max-width:1320px;margin:0 auto;padding:14px 28px;font-size:.75rem;color:var(--gray-400)">
    ...
    <a href="{{ route('appointments.index') }}" style="color:var(--primary);text-decoration:none;font-weight:500">Lịch hẹn</a>
    <span style="margin:0 6px">/</span>
...
<div class="page">
    ...
    <div class="main-col">
        ...
        {{-- Alerts --}}
        @if(session('success'))
        <div style="background:#e8f3ef;border-left:4px solid #059669;padding:14px 20px;border-radius:18px;margin-bottom:24px;font-size:.85rem;color:#065f46">
            ✔️ {{ session('success') }}
        </div>
        @endif
        @if($errors->has('msg'))
        <div style="background:#fee9e9;border-left:4px solid #e5484d;padding:14px 20px;border-radius:18px;margin-bottom:24px;font-size:.85rem;color:#991b1b">
            ⚠️ {{ $errors->first('msg') }}
        </div>
        @endif
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        @error('work_date')
                        <span style="font-size:.7rem;color:#e5484d;display:block;margin-top:4px">{{ $message }}</span>
                        @enderror
                    </div>

                    <div style="font-size:.7rem;font-weight:700;text-transform:uppercase;color:var(--gray-600);letter-spacing:.03em;margin:16px 0 4px">
                    ...
                    <div class="slot-legend" id="slot-legend" style="display:none">
                        <div class="legend-item"><div class="legend-dot"></div>Còn trống</div>
                        <div class="legend-item"><div class="legend-dot full-dot"></div>Đã đầy</div>
    ...
    <div class="sidebar">
        ...
        <div class="summary-card">
            ...
            <div class="sum-body">
                ...
                <div class="sum-row"><span class="k">Ngày</span>    <span class="v empty" id="sum-date" style="color:var(--primary);font-weight:700">—</span></div>
                <div class="sum-row"><span class="k">Giờ</span>     <span class="v empty" id="sum-time" style="color:var(--primary);font-weight:700">—</span></div>
