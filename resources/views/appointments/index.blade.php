                                    ...
                                    @elseif(in_array($item->status, ['Đã khám', 'Đã Khám', 'Hoàn thành', 'Hoàn Thành']))
                                        ...
                                        <div class="actions" style="flex-wrap:wrap">
                                            @if($item->review_id ?? null)
                                                ...
                                                @if($canEdit || $isAdmin)
                                                    <button type="button" class="btn-edit" data-review='@json(["appointmentId" => $item->appointment_id, "doctorId" => $item->doctor_id, "doctorName" => $item->doctor_name, "deptName" => $item->department_name, "workDate" => \Carbon\Carbon::parse($item->work_date)->format("d/m/Y"), "avatarUrl" => "", "storeUrl" => route("reviews.store"), "existing" => ["reviewId" => $item->review_id, "rating" => $item->review_rating ?? 0, "comment" => $item->review_comment ?? "", "updateUrl" => route("reviews.update", $item->review_id ?? 0)]])' onclick="openReviewModal(JSON.parse(this.dataset.review))">✏️ Sửa</button>
                                                @endif

                                                {{-- Xóa: chính user hoặc admin --}}
                                                @if($canEdit || $isAdmin)
                                                    <button type="button" class="btn-cancel" data-destroy-url="{{ route('reviews.destroy', $item->review_id ?? 0) }}" onclick="openDeleteReviewModal(this.dataset.destroyUrl)">
                                                        🗑 Xóa
                                                    </button>
                                                @endif
                                                ...
                                                @if($isAdmin || $isDoctor)
                                                    <button type="button" class="btn-edit" style="background:#f0fdf4;color:#16a34a;border-color:#bbf7d0" data-reply='@json(["replyUrl" => route("reviews.reply", $item->review_id ?? 0), "stars" => $item->review_rating ?? 0, "comment" => $item->review_comment ?? "", "userName" => auth()->user()->full_name ?? auth()->user()->name ?? "", "existingReply" => $item->doctor_reply ?? ""])' onclick="openReplyModal(JSON.parse(this.dataset.reply))">
                                            ...
                                            @else
                                                {{-- Chưa đánh giá --}}
                                                <button type="button" class="btn-edit btn-review" data-review='@json(["appointmentId" => $item->appointment_id, "doctorId" => $item->doctor_id, "doctorName" => $item->doctor_name, "deptName" => $item->department_name, "workDate" => \Carbon\Carbon::parse($item->work_date)->format("d/m/Y"), "avatarUrl" => "", "storeUrl" => route("reviews.store"), "existing" => null])' onclick="openReviewModal(JSON.parse(this.dataset.review))">⭐ Đánh giá</button>
                                            @endif
                                        </div>
