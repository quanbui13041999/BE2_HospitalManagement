{{-- resources/views/medical_history/index.blade.php --}}
@extends('layouts.app')
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="fw-bold mb-0">
        <i class="bi bi-folder-fill text-warning"></i> Lịch Sử Khám Của Tôi
    </h5>
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('medical_history.index') }}" id="searchForm">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text"
                           ...
                           placeholder="Mã phiếu, tên bác sĩ..."
                ...
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('medical_history.index') }}" class="btn btn-secondary btn-sm px-3">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0" style="font-size:13.5px">
            <thead style="background:#f8f9fa;font-size:12px;color:#666;">
                <tr>
                    <th>
                        <a href="{{ route('medical_history.index', array_merge(request()->all(), ['sort_by' => 'record_code', 'sort_order' => (request('sort_by') == 'record_code' && request('sort_order') == 'asc') ? 'desc' : 'asc'])) }}"
                    ...
                    <th>Bác sĩ</th>
                    <th>
                        <a href="{{ route('medical_history.index', array_merge(request()->all(), ['sort_by' => 'exam_date', 'sort_order' => (request('sort_by') == 'exam_date' && request('sort_order') == 'asc') ? 'desc' : 'asc'])) }}"
                           class="text-decoration-none text-dark">
                            Ngày khám
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($record->diagnoses->isNotEmpty())
                            {{ \Illuminate\Support\Str::limit($record->diagnoses->first()->diagnosis_name, 40) }}
                        @else
                ...
                <tr>
                    <td colspan="7" class="text-center text-muted py-5">
                        ...
                        @if(request()->filled('search') || request()->filled('visit_type') || request()->filled('status') || request()->filled('date_from') || request()->filled('date_to'))
                            <br>
                            <a href="{{ route('medical_history.index') }}" class="btn btn-sm btn-primary mt-2">
                                Xóa bộ lọc
                            </a>
                        @endif
                    </td>
            ...
            <div>
                ...
                <select class="form-select form-select-sm" style="width: auto;" onchange="window.location.href=this.value">
                    @foreach([10, 20, 50, 100] as $size)
                        <option value="{{ route('medical_history.index', array_merge(request()->all(), ['per_page' => $size])) }}"
