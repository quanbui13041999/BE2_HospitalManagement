...
@section('title', 'Thêm Hồ sơ tiêm chủng')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1"><i class="bi bi-plus-circle me-2 text-primary"></i>Thêm Hồ sơ tiêm chủng</h4>
            ...
            <form action="{{ route('admin.vaccination-records.store') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="user_id" class="form-select @error('user_id') is-invalid @enderror" required>
                            ...
                            @foreach($patients as $patient)
                                <option value="{{ $patient->user_id }}" {{ old('user_id') == $patient->user_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-8">
                        ...
                        <select name="vaccine_id" class="form-select @error('vaccine_id') is-invalid @enderror" required>
                            ...
                            @foreach($vaccines as $vaccine)
                                <option value="{{ $vaccine->vaccine_id }}" {{ old('vaccine_id') == $vaccine->vaccine_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="dose_number" class="form-control @error('dose_number') is-invalid @enderror" value="{{ old('dose_number', 1) }}" min="1" required>
                        @error('dose_number') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Ngày tiêm</label>
                        <input type="datetime-local" name="administered_at" class="form-control @error('administered_at') is-invalid @enderror" value="{{ old('administered_at') }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                            <option value="Chưa tiêm" {{ old('status') == 'Chưa tiêm' ? 'selected' : '' }}>Chưa tiêm</option>
                            <option value="Đã tiêm" {{ old('status') == 'Đã tiêm' ? 'selected' : '' }}>Đã tiêm</option>
