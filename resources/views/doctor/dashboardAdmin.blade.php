...
<body>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <a href="{{ route('home') }}" class="active">
                            <h1 class="text-xl font-bold text-gray-900"> MediCore<sup>®</sup></h1>
                        </a>
                ...
                @auth
                    @if(auth()->user()->is_admin ?? false)
                        <a href="{{ route('admin.dashboard') }}"
                            ...
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 0１１ １v6a１ １ ０ ０１－１ １h－２a１ １ ０ ０１－１－１v－６z" />
                            </svg>
                            Thống kê
                        </a>
                    @endif
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6 flex gap-2">
            <button class="time-range-btn active px-4 py-2 rounded-lg font-medium transition-colors"
                onclick="setTimeRange('week')">
                7 ngày
            </button>
            <button class="time-range-btn px-4 py-2 rounded-lg font-medium transition-colors"
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-sm font-medium text-green-600 bg-green-50 px-2 py-1 rounded">{{ $appointmentStats['total'] > 0 ? '+12%' : '0%' }}</span>
                    </div>
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-sm font-medium text-red-600 bg-red-50 px-2 py-1 rounded">{{ $appointmentStats['cancellation_rate'] }}%</span>
                    </div>
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-sm font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded">{{ count($topDoctors) }}</span>
                    </div>
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-sm font-medium text-cyan-600 bg-cyan-50 px-2 py-1 rounded">{{ $patientStats['returning_rate'] }}%</span>
                    </div>
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-sm font-medium text-gray-600 bg-gray-50 px-2 py-1 rounded">{{ $patientStats['male'] > 0 ? round(($patientStats['male'] / $patientStats['total']) * 100) : 0 }}%
                            /
                            {{ $patientStats['female'] > 0 ? round(($patientStats['female'] / $patientStats['total']) * 100) : 0 }}%</span>
                    </div>
                    <p class="text-sm text-gray-600 mb-1">Nam / Nữ</p>
                    <p class="text-3xl font-bold text-gray-900">{{ $patientStats['male'] }} /
                        {{ $patientStats['female'] }}
                    </p>
                </div>
            </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $performanceStats['avg_wait_time'] }} <span
                            class="text-lg text-gray-500">phút</span></p>
                </div>
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $performanceStats['avg_examination_time'] }} <span
                            class="text-lg text-gray-500">phút</span></p>
                </div>
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $performanceStats['avg_rating'] }} <span
                            class="text-lg text-gray-500">/5.0</span></p>
                </div>
                ...
                <div class="stat-card bg-white rounded-xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-sm font-medium text-violet-600 bg-violet-50 px-2 py-1 rounded">{{ $performanceStats['review_count'] }}
                            đánh giá</span>
                    </div>
                    <p class="text-sm text-gray-600 mb-1">Tỷ lệ phản hồi</p>
                    <p class="text-3xl font-bold text-gray-900">{{ $performanceStats['review_rate'] }} <span
                            class="text-lg text-gray-500">%</span></p>
                </div>
            </div>
        </div>
                        ...
                        <div>
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div class="bg-green-500 h-2 rounded-full"
                                    style="width: {{ min(($waitTimeData['wait_times'][$key] / 25) * 100, 100) }}%"></div>
                            </div>
                        </div>
                    @empty
        ...
        <!-- Top Doctors Table & Top Doctor This Week -->
        <div class="grid lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                @if(!empty($topDoctorWeek))
                    <div class="text-center py-4">
                        <img src="{{ $topDoctorWeek['avatar_url'] ?? 'https://via.placeholder.com/100' }}" alt="Doctor"
                        ...
                        <div class="bg-blue-50 rounded-lg p-3">
                            <p class="text-sm text-gray-600">
                                Được yêu thích nhất trong tuần này với đạt <span
                                    class="font-bold text-blue-600">{{ $topDoctorWeek['appointment_count'] }}</span> lượt
                                đặt.
                            </p>
                        </div>
                    </div>
